 ...
 */
package io.gravitee.resource.oauth2.entraid.jwk;
...
/**
 * Specific implementation of {@link JWKSource} allowing retrieve the JWKS from an url and triggering refresh every {@link #JWKS_REFRESH_INTERVAL} in background.
...
public class JWKSUrlJWKSourceResolver<C extends SecurityContext> implements JWKSource<C> {
    ...
    public JWKSUrlJWKSourceResolver(String jwksUrl, ContentRetriever contentRetriever) {
    ...
    public List<JWK> get(JWKSelector jwkSelector, C context) throws KeySourceException {
        ...
        if (cachedJWKSource != null) {
            if (cachedJWKSource.isCacheExpired(JWKS_REFRESH_INTERVAL)) {
                backgroundRefresh();
            }
